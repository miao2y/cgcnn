...
def get_one_graph(adj, features, fea_limits=None):
    fea = []
    for i in range(1, len(features)):
        if fea_limits:
            fea.append(features[i].split()[2:2 + fea_limits])
        else:
...
def data_process(adj_path, feature_path, com_feature_path, model_name, mask, seed):
    ...
    for i, f in enumerate(os.listdir(feature_path)):
        ...
        with open(feature_path + f, 'r') as res:
            ...
            features_na = normalizer(g_mask.ndata['atom_features'].numpy())  # norm for node feature
